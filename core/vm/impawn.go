...
func (i *ImpawnImpl) reward(height uint64, allAmount *big.Int) ([]*types.SARewardInfos, error) {
	einfo := types.GetEpochFromHeight(height)
	return i.calcReward(height, allAmount, einfo)
}
...
func (i *ImpawnImpl) Reward(height uint64, allAmount *big.Int) ([]*types.SARewardInfos, error) {
	res, err := i.reward(height, allAmount)
	if err == nil {
		i.lastReward = height
	}
	return res, err
