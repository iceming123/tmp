...
func New(ctx *node.ServiceContext, config *etrue.Config) (*LightEtrue, error) {
	...
	if leth.fblockchain, err = fast.NewLightChain(leth.odr, leth.chainConfig, leth.engine, nil); err != nil {
		return nil, err
	}
	// Note: NewLightChain adds the trusted checkpoint so it needs an ODR with
	// indexers already set but not started yet
	if leth.blockchain, err = light.NewLightChain(leth.fblockchain, leth.odr, leth.chainConfig, leth.engine, nil); err != nil {
		return nil, err
	}
	leth.election = NewLightElection(leth.fblockchain)
	...
	if leth.protocolManager, err = NewProtocolManager(leth.chainConfig, nil, public.DefaultClientIndexerConfig, nil, 0, true, config.NetworkId, leth.eventMux, leth.engine, leth.peers, leth.fblockchain, leth.blockchain, nil, chainDb, leth.odr, leth.serverPool, nil, quitSync, &leth.wg, leth.election, nil); err != nil {
		return nil, err
	}
